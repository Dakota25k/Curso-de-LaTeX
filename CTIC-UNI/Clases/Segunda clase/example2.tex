% !TeX document-id = {3a76180a-e30e-4f5d-9fbb-ced79b3e9951}
% !TeX TXS-program:compile = txs:///pdflatex/[--shell-escape]
\documentclass{article}
\usepackage[utf8]{inputenc}
\usepackage[spanish, es-sloppy]{babel} 		% En el caso que se nombre más de un idioma, prevalece el último idioma.
\usepackage[dvipsnames, usenames]{color}
\usepackage{lipsum}							% Generar texto
\usepackage[pdftex]{graphicx}
\graphicspath{{../../images/}}				% Ruta de las imágenes
\usepackage{here}							% Paquete obsoleto, permite inserción de float.

\title{Mi primer documento}
\author{Oromion\thanks{Lima - Perú}\\UNI\and y el otro\\CTIC}
\date{hoy día}

\renewcommand{\spanishlisttablename}{Mis tablas}	% Comandos globales, válidos cuando se utiliza el paquete babel con opción spanish.
\renewcommand{\spanishabstractname}{Abstracto2}		% Comandos globales, válidos cuando se utiliza el paquete babel con opción spanish.

\pagestyle{empty}

\begin{document}
%TODO Leer el libro El Universo LaTeX de Rodrigo Castro.
\maketitle
\thispagestyle{empty}
...
\newpage
\vspace*{4cm}	% Si en la página actual no se encuentra texto antes de usar el comando.
Hola.

\vspace{2cm}	% Si en la página actual se encuentra texto antes de usar el comando.
...
\textsl{Universidad Nacional de Ingeniería}	% Slan quiere decir oblicua.

{\slshape Universidad Nacional de Ingeniería}

\textsc{Universidad Nacional de Ingeniería} % Small Caps
...
\includegraphics[width=4cm]{logo.png}			% No es necesario colocar la extensión de la imagen.
\includegraphics[height=2cm]{logo}				% El ancho o el alto se mantiene en proporción con el anterior.
\includegraphics[width=4cm,height=2cm]{logo}	% Hay deformación.

\includegraphics[scale=0.3]{logouni}			% Reescalando imagen.
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				\item 1
				\item 2
%				\begin{enumerate}		% Si se supera el nivel 4 de anidamientos se obtiene un error.
%					\item a
%					\item b
%					\item c
%				\end{enumerate}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item 5
				\item 6
%				\begin{itemize}			% Si se supera el nivel 4 de anidamientos se obtiene un error.
%					\item you
%				\end{itemize}
				\item 3
			\end{itemize}
...
\begin{table}[H] %con H se evita que flote
	\centering
	\caption{Esto es una tabla $n^{\circ}2$}\label{tab:2}% Se estila que el caption esté en la parte superior.
...
Mi ecuación es (\ref{eq:1}).% Más adelante utilizaremos: \eqref{eq:1}.
